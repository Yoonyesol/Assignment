    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">모든 가맹 계약 문서와 진행 상황을 통합 관리합니다.</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-white">
                ...
                <th className="py-4 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-[40%]">문서명</th>
                <th className="py-4 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">가맹점</th>
                <th className="py-4 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">상태</th>
                <th className="py-4 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">최근 활동</th>
...
const ContractRow = ({ title, label, store, owner, status, date }: any) => {
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors group">
      ...
      <td className="py-4 px-4 align-top">
        <div className="text-sm text-gray-900 font-medium">{store}</div>
        <div className="text-xs text-gray-500 mt-0.5">{owner} 점주</div>
      </td>
      <td className="py-4 px-4 align-top">
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 origin-top-right rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 z-50 focus:outline-none animate-in fade-in zoom-in-95 duration-100">
              <div className="py-1">
                ...
                <button
                   className="w-full text-left flex items-center px-4 py-2.5 text-xs text-red-600 hover:bg-red-50"
